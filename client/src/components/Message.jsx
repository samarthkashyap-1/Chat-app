...
import { getGroupChatById } from "../Apis/api";
...
const Message = ({ message, currentUser }) => {
  ...
  useEffect(() => {
    ...
    const Groupdata = async () => {
      const data = await getGroupChatById(message.groupId);
      setSenders(data.members);
    };
    ...
    >
      <div className="flex gap-2">
        ...
        >
          {senders &&
            senders.map((sender) => {
              if (sender.id === message.sender && user.id !== sender.id) {
                return (
          ...
          {/* Render file message if message type is 'file' */}
